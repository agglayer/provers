...
use crate::proposer_v1::{
    proposer_service_client::ProposerServiceClient, GetProofsRequest, GetProofsResponse,
};
use crate::{Request, Response};

pub type ProposerGrpcClient = ProposerServiceClient<tonic::transport::Channel>;
...
pub(crate) fn connect_proposer_service(
    ...
    match Endpoint::from_str(&endpoint) {
        Ok(endpoint) => Ok(ProposerServiceClient::new(endpoint.connect_lazy())),
        Err(e) => {
            error!(endpoint, "Failure to setup the gRPC API with endpoint: {e}");
            Err(e)
        }
    }
}
