...
use crate::error::ProofRequestError;

use super::{
    error::GrpcConversionError, AggSpanProofProposerRequest, AggSpanProofProposerResponse,
};
...
impl TryFrom<grpc::AggProofResponse> for AggSpanProofProposerResponse {
    ...
    fn try_from(response: grpc::AggProofResponse) -> Result<Self, Self::Error> {
        if response.success {
            let (proof_id, start_block, end_block) = todo!();
